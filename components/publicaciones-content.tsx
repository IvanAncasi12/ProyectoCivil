...
interface PublicacionesContentProps { colors: { primario: string; secundario: string; terciario: string; }; }
...
export default function PublicacionesContent({ colors }: PublicacionesContentProps) {
  ...
  if (loading) return <section className="py-24 bg-slate-950 min-h-screen flex items-center justify-center"><div className="text-2xl font-semibold animate-pulse" style={{ color: colors.primario }}>Cargando publicaciones...</div></section>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          <div className="lg:col-span-3 space-y-8">
            {publicaciones.length === 0 ? (
              <div className="tech-card p-16 text-center"><FileText className="w-16 h-16 mx-auto mb-4 opacity-30" style={{ color: colors.primario }} /><h3 className="text-2xl font-bold text-white mb-2">No hay publicaciones disponibles</h3><p className="text-slate-400">Pronto compartiremos nuevos artículos</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {publicaciones.map((pub) => (
                  <div key={pub.publicaciones_id} className="tech-card p-6 hover:scale-[1.02] transition-transform" style={{ borderColor: `${colors.primario}30` }}>
                    <div className="flex flex-col gap-4">
                      <div className="w-full h-48 rounded-xl overflow-hidden cursor-pointer group" onClick={() => pub.publicaciones_imagen && setSelectedImage(pub.publicaciones_imagen)}>
                        {pub.publicaciones_imagen ? <img src={pub.publicaciones_imagen} alt={pub.publicaciones_titulo} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center bg-slate-800"><FileText className="w-12 h-12 opacity-30" style={{ color: colors.primario }} /></div>}
                      </div>
                      <div>
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-2" style={{ backgroundColor: colors.primario }}>{pub.publicaciones_tipo || 'Artículo'}</span>
                        <h3 className="text-xl font-bold text-white mb-2">{pub.publicaciones_titulo}</h3>
                        <p className="text-slate-300 text-sm mb-4 line-clamp-2">{pub.publicaciones_descripcion}</p>
                        <div className="flex flex-wrap gap-4 text-sm text-slate-400">
                          {pub.publicaciones_fecha && <div className="flex items-center gap-2"><Calendar className="w-4 h-4" style={{ color: colors.secundario }} /><span>{formatDate(pub.publicaciones_fecha)}</span></div>}
                          {pub.publicaciones_autor && <div className="flex items-center gap-2"><Users className="w-4 h-4" style={{ color: colors.terciario }} /><span>{pub.publicaciones_autor}</span></div>}
                        </div>
                        {pub.publicaciones_documento && (
                          <a href={pub.publicaciones_documento} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold hover:underline" style={{ color: colors.primario }}>
