...
import { 
  ...
  TrendingUp, Award, Users, Briefcase, X, AlertCircle
} from 'lucide-react';
  ...
  if (loading) {
    return (
      <section className="py-24 bg-slate-950 min-h-screen flex items-center justify-center">
        <div className="text-2xl font-semibold animate-pulse" style={{ color: colors.primario }}>Cargando avisos...</div>
      </section>
    );
  }
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            {avisos.length === 0 ? (
              <div className="tech-card p-16 text-center">
                <AlertCircle className="w-16 h-16 mx-auto mb-4 opacity-30" style={{ color: colors.terciario }} />
                <h3 className="text-2xl font-bold text-white mb-2">No hay avisos disponibles</h3>
                <p className="text-slate-400">Pronto publicaremos nuevos avisos</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6">
                {avisos.map((aviso) => (
                  <div 
                    ...
                    className="tech-card p-6 hover:scale-[1.01] transition-transform border-l-4"
                      ...
                      >
                        {aviso.con_foto_portada ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-slate-800">
                            <AlertCircle className="w-12 h-12 opacity-30" style={{ color: colors.terciario }} />
                          </div>
                        )}
                      </div>
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-slate-400">
                          {aviso.con_fecha_inicio && (
                            <div className="flex items-center gap-2">
                              ...
                              <span>{formatDate(aviso.con_fecha_inicio)}</span>
                            </div>
                          )}
                        </div>
                      </div>
