...
import { Calendar, Clock, MapPin, FileText, ExternalLink, TrendingUp, Award, Users, Briefcase, X } from 'lucide-react';
import { api, Evento } from '@/lib/api';

interface EventosContentProps { colors: { primario: string; secundario: string; terciario: string; }; }
...
export default function EventosContent({ colors }: EventosContentProps) {
  ...
  if (loading) return <section className="py-24 bg-slate-950 min-h-screen flex items-center justify-center"><div className="text-2xl font-semibold animate-pulse" style={{ color: colors.primario }}>Cargando eventos...</div></section>;
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          <div className="lg:col-span-3 space-y-8">
            {eventos.length === 0 ? (
              <div className="tech-card p-16 text-center"><Calendar className="w-16 h-16 mx-auto mb-4 opacity-30" style={{ color: colors.primario }} /><h3 className="text-2xl font-bold text-white mb-2">No hay eventos disponibles</h3><p className="text-slate-400">Pronto anunciaremos nuevas actividades</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {eventos.map((evento) => (
                  <div key={evento.evento_id} className="tech-card p-6 hover:scale-[1.02] transition-transform" style={{ borderColor: `${colors.primario}30` }}>
                    <div className="flex flex-col gap-4">
                      <div className="w-full h-48 rounded-xl overflow-hidden cursor-pointer group" onClick={() => evento.evento_imagen && setSelectedImage(evento.evento_imagen)}>
                        {evento.evento_imagen ? <img src={evento.evento_imagen} alt={evento.evento_titulo} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center bg-slate-800"><Calendar className="w-12 h-12 opacity-30" style={{ color: colors.primario }} /></div>}
                      </div>
                      <div>
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold text-white mb-2" style={{ backgroundColor: colors.primario }}>{evento.tipo_evento || 'Evento'}</span>
                        <h3 className="text-xl font-bold text-white mb-2">{evento.evento_titulo}</h3>
                        <p className="text-slate-300 text-sm mb-4 line-clamp-2">{evento.evento_descripcion}</p>
                        <div className="flex flex-wrap gap-4 text-sm text-slate-400">
                          {evento.evento_fecha && <div className="flex items-center gap-2"><Calendar className="w-4 h-4" style={{ color: colors.secundario }} /><span>{formatDate(evento.evento_fecha)}</span></div>}
                          {evento.evento_hora && <div className="flex items-center gap-2"><Clock className="w-4 h-4" style={{ color: colors.terciario }} /><span>{evento.evento_hora}</span></div>}
                          {evento.evento_lugar && <div className="flex items-center gap-2"><MapPin className="w-4 h-4" style={{ color: colors.primario }} /><span>{evento.evento_lugar}</span></div>}
                        </div>
                      </div>
                    </div>
                  </div>
